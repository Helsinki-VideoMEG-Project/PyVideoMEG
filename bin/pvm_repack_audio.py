...
def main():
    ...
    print(
        "New file: %i buffers of the size %i, last %i samples discarded"
        % (new_buf_cnt, new_buf_sz, int((old_buf_sz * old_buf_cnt) % new_buf_sz))
    )
    ...
    out_file.write(
        struct.pack("I", aud_file.ver)
    )  # preserve original file format version

    out_file.write(struct.pack("I", aud_file.srate))
    out_file.write(struct.pack("I", aud_file.nchan))
    if aud_file.ver == 0:
        out_file.write(
            aud_file.format_string.encode("ascii")
        )  # preserve format_string for version 0
    ...
    for i in range(new_buf_cnt):
        ...
        out_file.write(
            aud_file.raw_audio[
                i * new_buf_sz * (aud_file.nchan * 2) : (i + 1)
                * new_buf_sz
                * (aud_file.nchan * 2)
            ]
        )
