...
def main():
    ...
    assert (vid_file_1.ts[0] < vid_file_2.ts[-1]) & (
        vid_file_2.ts[0] < vid_file_1.ts[-1]
    )

    # skip the unmatched frames (if any) at the beginning of video_file_1
    i = 0
    while min(abs(vid_file_1.ts[i + 1] - vid_file_2.ts)) < min(
        abs(vid_file_1.ts[i] - vid_file_2.ts)
    ):
        i += 1
    ...
    while i < len(vid_file_1.ts):
        ...
        draw.text(
            (10, 0),
            "%i  :  %s" % (vid_file_1.ts[i], pyvideomeg.ts2str(vid_file_1.ts[i])),
            font=fnt,
            fill="black",
        )
    ...
    print(
        "File #2: used %i frames (%i distinct) out of %i, %i skips, %i frames reused"
        % (len(indx), distinct, len(vid_file_2.ts), skipped, reused)
    )
    print(
        "Matching error: average: %f ms, absolute average: %f ms, maximal: %f ms"
        % (numpy.mean(err), numpy.mean(abs(err)), max(abs(err)))
    )
